 ...
 * — usually Victorian/Edwardian ornate atomizers with multi-tier finials.
 * These descriptors anchor each applicator type in Best-Bottles-specific
 * visual language with explicit constraints (collar proportions, finial
 * size, etc.) so the model lands in the right product category.
 *
 * 2026-04-29 CORRECTION: original descriptors said "NO hose between collar
 * and bulb." This was wrong. Live PDP photos confirm Best Bottles' bulb
 * sprayers (Vintage AND Antique families) DO have a short braided silk
 * hose connecting the bulb to the collar. The hose color matches the bulb
 * color and connects via a small chrome ring at each end. The descriptors
...
export const APPLICATOR_SHAPE_DESCRIPTORS: Record<string, string> = {
  "Vintage Bulb Sprayer":
    "Silver chrome collar with a soft mesh squeeze bulb connected to one side of the collar VIA AN ARCHING BRAIDED SILK HOSE. " +
    ...
    "HOSE — MANDATORY: the mesh bulb is connected to the SIDE of the collar (close to the level of the top plate) via a short braided silk hose that arches gracefully UP and OUT from the collar then DOWN to the bulb, like a gentle question-mark or J-curve. The hose is the SAME COLOR as the mesh bulb (so for a red bulb the hose is red; for black the hose is black). The hose attaches with a small polished chrome connector ring at the collar end AND a matching chrome connector ring at the bulb end. The hose length should be roughly 1.5–2× the bulb diameter so it forms a visible arch, not a stub. " +
    "NO tassel. NO Victorian or Edwardian multi-tier ornamentation. NO dip tube in this layer (tube belongs to the body layer). " +
    "Modern apothecary design with classic bulb-sprayer hose silhouette.",

  "Vintage Bulb Sprayer with Tassel":
    "Identical to Vintage Bulb Sprayer (SHORT DISK-shaped silver chrome collar — wider than tall — with knurled bottom edge, flat top with TINY low-profile dome finial, mesh bulb connected to one SIDE of the collar via a short ARCHING BRAIDED SILK HOSE matching the bulb's color, with chrome connector rings at each end), " +
    "PLUS a decorative silk tassel of the specified color hanging DOWN from the bulb's bottom edge (the same place the hose enters the bulb on the opposite side). " +
    "Tassel is a simple braided silk fringe, modest length (about 1.5–2× the bulb diameter), NOT ornate. The tassel is the same color as the bulb and hose. " +
    "HOSE detail: short braided silk arching from the collar's side UP and OUT then DOWN to the bulb in a gentle J-curve. Same color as bulb. Chrome connector rings at both ends. " +
    "NO Victorian multi-tier ornamentation. NO multi-tier finials. NO dip tube in this layer. " +
    "Collar proportions are critical: SHORT, DISK-shaped, wider than tall — NOT a tall column or chalice.",

  "Antique Bulb Sprayer":
    "Same as Vintage Bulb Sprayer — SHORT DISK-shaped silver chrome collar (wider than tall, knurled bottom, flat top, tiny low-profile dome finial), mesh squeeze bulb connected to one side of the collar via a short arching braided silk hose matching the bulb's color (chrome connector rings at each end). No tassel, no dip tube in this layer. Modern apothecary design with classic bulb-sprayer hose silhouette.",

  "Antique Bulb Sprayer with Tassel":
    "Same as Vintage Bulb Sprayer with Tassel — SHORT DISK-shaped silver chrome collar with knurled bottom, flat top, tiny dome finial, mesh squeeze bulb connected to one side via a short arching braided silk hose matching the bulb's color (chrome connector rings at each end), and a simple silk tassel hanging DOWN from the bulb's bottom in the same color. No dip tube in this layer.",
